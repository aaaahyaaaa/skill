...
不要打印原始 token、API key 或 Authorization header。OpenPlat ZS Open 固定 token 只通过 `OPEN_PLAT_ZS_OPEN_TOKEN` 配置，值不带 `Bearer`；CLI 会自动拼接 `Authorization: Bearer <token>`，并固定发送 `x-zs-plt-open: zs_open`。提交到仓库的默认配置不得包含真实密钥。
