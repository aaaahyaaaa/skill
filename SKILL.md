...
**Skill release marker:** `findreason-rag-attribution@2026-07-01-lean-artifacts-v1`

使用日志中必须能搜到这个 marker，或在 `python3 -m findreason schema` / `case_facts.json` / `evidence_index.json` 中看到同名字段 `skill_release_marker`，才说明当前执行面加载到了这版 JSON-only workflow-aware skill。
...
- 输出 `case_facts.json` 和按需生成的 `*_experiment.json`；新 run 不再落盘 `case.json` 或 `agent_brief.md`。
...
- 根据 `trace.workflow_topology` / `trace.node_evidence_map` / `trace.agent_span_read_plan` 建立 RAG 阶段产物地图，决定哪些节点和 span 值得深读；不要把脚本的候选读取建议当成最终 cause。

## RAG 阶段优先原则

真实 workflow 节点是定位索引，不是最终分析对象。Agent 的核心任务是借助模型理解不同 workflow 的业务形态，把 trace 中真正有用的 input/output 映射到 RAG 阶段产物，而不是机械给每个 workflow 节点贴标签。

先建立一张 case-specific 的 `RAG stage map`，至少覆盖：

- `query/input`：用户原问、Workflow input、query_list、chat_history 是否进入当前问题。
- `preprocess/rewrite`：rewrite query、keywords、子问题、意图分类、条件分支入参。
- `recall`：文档召回、FAQ/精选召回、各分支召回、HTTP/API recall/searchDoc 响应。
- `rerank`：重排输入输出、rank/score、过滤、merge、各渠道均分或去重。
- `prompt/context`：进入知商问答、大模型、脚本拼 prompt 或其他 context builder 的证据；字段可能叫 `prompt_docs`、`promptDocs`、`qaPromptDocs`，也可能藏在模型 input、messages、doc_string 或脚本 output 里。
- `generation`：大模型/知商问答节点的真实 prompt、answer、模型输出。
- `postprocess/final_output`：脚本后处理、格式化、裁剪、包装层、结束节点输出、agent_reply。
- `evaluator/judged_object`：评估器实际评估的是 Workflow 输出、包装后答案，还是其他字段。

不要被节点类型卡住：一个节点可以承载多个 RAG 阶段，一个 RAG 阶段也可以由多个节点共同完成；脚本节点通常是规范化/拼接/后处理，但它也可能是真实 prompt-entry 边界或最终输出改写点。遇到 `script`、`branch`、`external_tool`、`unknown` 时，先读它的 input/output 和上下游，再判断它承载了哪个 RAG 阶段产物。

常见 workflow 识别口径：

- 标准 RAG QA：预处理 -> 召回 -> 重排 -> 知商问答 -> 结束。重点看 `recall -> rerank -> prompt/context -> answer` 的同一 required assertion 生存情况。
- 自定义大模型链路：召回/重排 -> 脚本拼 prompt -> 大模型 -> 后处理 -> 结束。重点看脚本是否把 rerank 证据放进模型 input，以及后处理是否改写/截断答案。
- 多分支召回/候选生成：条件分支 -> 多个召回节点 -> merge/均分/重排 -> 脚本/结束。它可能没有 LLM prompt；不要把 `prompt not_observed` 写成零证据生成，重点看候选覆盖、分支路由、merge/rerank 和最终候选输出。
- 工具/API 增强链路：RAG 节点与 HTTP/API/MCP/外部工具混合。重点看外部工具返回是否成为 recall 证据、prompt context 或最终答案依据。

## 主流程

1. 用 `collect-evidence` 固化 trace 和 RAG artifacts。
2. 只从 JSON 取证：定点读取 `case_facts.json` / `evidence_index.json` / `*_experiment.json` 中的相关字段，确认真实 input/output、证据列表、rank/score、prompt/context 和最终输出；不要直接通读完整大 JSON。
3. 不读取旧 run 里的 `agent_brief.md` 或 `agent_judgement.md` 作为分析输入。`agent_judgement.md` 是输出产物，不能作为证据、导航、历史参考或 cause 依据；`agent_brief.md` 已从新产物契约中移除。
4. 定点读取的核心 JSON 路径包括：`case`、`trace.workflow_topology`、`trace.node_evidence_map`、`trace.prompt_observation`、`trace.agent_span_read_plan`、`artifacts.*`、`raw_trace_evidence.*`、`experiment_inputs.*`、`rank_shift_observations` 和各类 `*_experiment.json`。
5. 明确本 case 的 RAG stage map：每个阶段对应哪些 span、关键字段、产物数量、置信度和仍缺证据。
6. 抽答案症状，再列候选根因。
7. 对每个候选根因写明支持证据、反证证据和下一步实验。
8. 需要时用 `run-experiment` 规划或运行 recall / rerank / replay / knowledge-detail 实验。
9. 运行 `synthesize-brief` 生成短版 judgement summary 和证据索引。
10. Agent 基于事实和实验结果给用户输出短版结论，风格类似一次清晰复盘回复；不要要求用户阅读 JSON 或长报告。
...
- 必需：本地 `python3 -m findreason` CLI、`case_facts.json`、`evidence_index.json`、`*_experiment.json`。
- 输出：`agent_judgement.md` 只能作为交付产物写入，不能作为分析输入或证据来源；`agent_brief.md` 是旧 run 兼容产物，新 run 不再生成。
...
因此，最稳的默认路径是：单 case source context + 本地 trace 文件 + `collect-evidence` 生成当前 `case_facts.json`，再按需生成 `evidence_index.json` / 实验 JSON，由宿主 LLM 定点读取 JSON 证据、建立 RAG stage map 并完成最终判断。source context 必须嵌入 `case_facts.json.case`，新 run 不再额外落盘 `case.json`；`agent_judgement.md` 是输出文件，不得反哺分析。只有用户明确要“重新拉线上 trace / 重跑 recall / 重跑 workflow”时，才启用对应可选能力。

如果用户要求重新分析、验证是否使用最新 skill、或 skill/parser 最近更新过，应忽略已有 Markdown 结论文件，直接基于当前 JSON 事实包重新判断。若 JSON 里缺少 `trace.workflow_topology`、`trace.node_evidence_map`、`trace.prompt_observation` 或 `trace.agent_span_read_plan`，应重新运行 `collect-evidence` 或明确写出该 JSON 证据缺口；不得用旧 Markdown 补证。
...
python3 -m findreason collect-evidence \
  ...
  --log-id <log_id> \
  --app-id <app_id> \
  --case-json '@/path/to/source_case_payload.json' \
...
`--case-file /path/to/case.json` 只保留为 legacy 兼容入口。新批处理不要生成持久 `case.json`；如果已有表格行/用户上下文，直接通过 `--case-json` 传入并让它进入 `case_facts.json.case`。
...
不传 `--query` / `--context-query` 时，recall 实验只输出 query variant 规划和历史 baseline。规划必须区分 `workflow_rewrite` 与 `verbatim_user`：`workflow_rewrite` 只来自 `preprocess.rewrite_query` 或 `trace.summary.workflow_segments[].query`，不能用 `case.query` 兜底；`verbatim_user` 才是用户原话，通常来自 `case.query`。历史 workflow query 的召回结果已经在 trace 中，应优先读取 `case_facts.json.artifacts.origin_doc_list` / `origin_faq_list` 和 `baseline_trace_recall`，不要再为同一个 workflow query 生成一份 `recall_experiment_workflow_query.json`。只有 query 与历史 workflow query 不同，或显式放宽 topK/label/threshold 时，才把结果保存为 counterfactual recall 实验。

传入 `--query` 且 trace 中有 recall/searchDoc 请求模板时，代码会输出 `recall_variant_matrix`：`baseline_trace_recall` 只总结历史 `origin_doc_list + origin_faq_list`，`workflow_query_override` 复用 trace template 执行，`topK/maxCount_relaxed` 默认把可识别 topK/maxCount 提到至少 50，`label_relaxed` / `threshold_relaxed` 只有在模板中明确找到字段时执行，否则标记 `unsupported`。如果模板里存在 `recallLabels` / label/tag 约束，还会按真实 query source 追加 `workflow_rewrite_no533_wide` 和/或 `verbatim_user_no533_wide`：文件名或 variant id 必须反映实际 query source；每个宽召回变体都会写 `actual_wide_query_used` 和 `same_assertion_gate.valid_recall_improvement`，表格写回时建议同步写入或内嵌 `actual_wide_query_used` 与 `valid_recall_improvement=false/true`。

宽召回只能作为探索证据，不能自动改变归因。只有 top 文档通过“同断言门槛”时，才允许把结论改成 `召回遗漏`：必须同时命中业务对象、问题类型、关键限定条件，并且正文能回答本 case 的 required assertion。宽召回 top 文档必须按 `direct_support` / `partial_support` / `adjacent_support` / `irrelevant` 四档判定；只有 `direct_support` 或强 `partial_support` 才能支撑 `召回遗漏`，`adjacent_support` / `irrelevant` 只能写成探索线索。像“行业参考值页面空白”这种问题，如果宽召回只命中“空白/参考值”的词面而不是同主题页面空白规则，`valid_recall_improvement` 必须是 `false`。

如果原 prompt / prompt_docs 已经含有能回答关键断言的文档，而 Workflow 输出仍然写错，应优先判断 `答案生成错误`、`证据使用错误` 或 `知识冲突`，不能因为宽召回又找到一个更清晰的文档就自动改成 `召回遗漏`。
...
`--version-id` / `--app-version` 只用于 replay 的 app-detail 解析。用户提供时会作为 `appVersion` 传给 `/open-plat/api/app/get-app-detail`；用户没有提供时不传 `appVersion`，由平台接口返回最新版本。replay 只在存在明确 counterfactual query、forced/selected core doc、或历史 trace 缺少中间节点证据时执行；如果只是 `skipped_authoritative_trace`，优先把状态写入 manifest / judgement 摘要，不需要为每条 case 强制落空的 `replay_experiment.json`。
...
- Workflow-aware 侦查：优先使用 `case_facts.json.trace.workflow_topology` 的 app-detail 真实节点信息作为索引；`node_evidence_map[].inferred_role` 只是辅助说明。Agent 应根据真实节点 type/name、span input/output、上下游关系和证据字段位置，把节点映射到 RAG stage map，再决定深读哪些 span。
...
`agent_judgement.md` 是结论文件，不是提示词或 checklist。不要在里面写“Agent 最终回复要求”“给用户输出短版结论”这类面向 Agent 的写作要求；这类规则只保留在 `SKILL.md` 或 `references/report_contract.md`。
...
- 默认使用本地 JSON 作为可索引事实来源：`case_facts.json`、真正的 counterfactual `*_experiment.json`、`evidence_index.json`。历史 workflow query 的召回结果属于 `case_facts.json`，不是实验文件；空 replay / skipped replay 不应强制落成实验文件。
