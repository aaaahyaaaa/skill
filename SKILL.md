...
**Skill release marker:** `findreason-rag-attribution@2026-06-29-json-only-workflow-aware-v1`
...
3. 不读取 `agent_brief.md` 或 `agent_judgement.md` 作为分析输入。二者都是输出产物，不能作为证据、导航、历史参考或 cause 依据。
...
- 输出：`agent_brief.md`、`agent_judgement.md` 只能作为交付产物写入，不能作为分析输入或证据来源。
...
因此，最稳的默认路径是：单 case 本地 case 文件 + 本地 trace 文件 + `collect-evidence` 生成当前 `case_facts.json` / `evidence_index.json` / 实验 JSON，再由宿主 LLM 定点读取 JSON 证据、建立 RAG stage map 并完成最终判断。`agent_brief.md` 和 `agent_judgement.md` 都是输出文件，不得反哺分析。只有用户明确要“重新拉线上 trace / 重跑 recall / 重跑 workflow”时，才启用对应可选能力。
