...
20. `host_agent.answer_claim` 是向后兼容字段，语义上表示宿主 Agent 产出的 assertion set。使用嵌套 JSON 结构 `{"host_agent": {"answer_claim": [...]}}`。每项应为对象，包含 `text`、`role`，可选 `basis`、`why_required`、`source`、`confidence`；核心 role 是 `expected_required` 和 `answer_claim`。`missing_expected` 仅作为兼容输入，宿主应归一化为 `expected_required` 并保留遗漏 hint。`constraint_check`、`citation_check`、`consistency_check` 只用于 `probe-v1` 计划（范围、引用、一致性探针），不直接驱动上游归因。CLI 会把 `source` 归一化为 `host_agent.answer_claim`。
21. `expected_required` 由宿主 Agent 基于 trace query、chat_history、评估器 reason、rewrite query、keywords 等上下文推断，表示正确输出应覆盖的检查点；它是归因靶子，不是事实证据。`answer_claim` 必须是从 workflow output 中抽取出的可验证命题 X，不能写成“答案称 X”的元叙述。
22. 外部传入的 `query` / `answer` 只是 hint。真实 query、rewrite query、keywords、output、召回、重排、prompt/context、引用和脚本 I/O 以 trace artifacts 为权威；只有 trace 缺失时才降级使用 hint，并降低证据质量或触发人工复核。
