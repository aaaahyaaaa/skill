...
`agent_brief.md` 是输出产物，不是分析输入。它可以展示 case-specific 的简明摘要，但 Agent 不能读取它作为证据、导航或结论依据。
...
证据必须来自 JSON，但不需要直接翻完整大 JSON。正确做法是定点读取 `case_facts.json`、`evidence_index.json` 或 `*_experiment.json` 的相关字段，例如 `trace.workflow_topology`、`trace.node_evidence_map`、`trace.prompt_observation`、`trace.agent_span_read_plan`、`artifacts.*`、`raw_trace_evidence.*`、`rank_shift_observations`。不要把“打开完整 case_facts.json 从头读到尾”当成默认动作，也不要读取 `agent_brief.md` / `agent_judgement.md` 作为证据、导航或历史参考。
...
不传 `--query` / `--context-query` 时只规划 query variants 并输出历史 `baseline_trace_recall`。传入 `--query` 且 `case_facts` 中有 `experiment_inputs.recall_templates` 时，会输出 `recall_variant_matrix`：`workflow_query_override`、`topK/maxCount_relaxed`、`label_relaxed`、`threshold_relaxed`。topK/maxCount 变体默认把可识别字段提到至少 50；label/threshold 只有模板里能明确定位字段才执行，否则标记 `unsupported`。`--context-query` 可重复，用于 query_list/chat_history 改写后的 query 观察；结果只描述召回命中、过滤和证据覆盖，不直接输出根因，也不自动上调 `输入侧问题`。
