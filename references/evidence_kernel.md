...
真实调用 workflow replay，返回当前版本运行下的 origin/rerank/prompt/answer 证据。replay 的线上链路只走接口：OpenPlat app detail API 获取 `workflowConfigV2`，OpenPlat workspace info API 获取 workspace 级 `authInfo.apiKey`，open-exec workflow API 执行重跑。用户提供 `--version-id` / `--app-version` 时作为 `appVersion` 传给 app detail；未提供时不传 `appVersion`，由平台接口取最新版本。replay 是实验结果，不覆盖原始 Fornax 现场。
