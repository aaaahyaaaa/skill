...
2. 如果命中已有再生 run，先读现成 JSON 证据包，而不是重跑一遍相同链路。本案例命中 `/Users/bytedance/Documents/findreason-rag-attribution-runs/20260622_lark_row36_latest`；旧目录里可能同时存在 `agent_brief.md` / `agent_judgement.md`，但当前分析只从 `case_facts.json`、真实 counterfactual experiment JSON 和 `evidence_index.json` 定点取证。
...
5. 核查 replay 状态。本案例是 `skipped_authoritative_trace`：再生 trace 已有中间节点证据，所以未执行新的 workflow replay，未返回新的 replay log_id；新 run 不需要为这个状态强制落一个空 `replay_experiment.json`。
