...
2. 不读取 `agent_brief.md` 或 `agent_judgement.md` 作为分析输入。二者都是输出产物，不能作为证据、导航、历史参考或 cause 依据。
...
`agent_brief.md` 和 `agent_judgement.md` 是输出文件，不是取证入口。已有 Markdown 不能用于检查、补证、继承 cause 或判断 stale；如果需要重新分析，直接基于当前 JSON 事实包重判。
