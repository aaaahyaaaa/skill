...
1. 只从 JSON 取证：定点读取 `case_facts.json`、`evidence_index.json`、`*_experiment.json`。不要默认通读完整大 JSON，但最终判断必须回到 JSON 中的真实 input/output、证据列表、rank/score、prompt/context 和最终输出。
2. 不读取旧 run 里的 `agent_brief.md` 或 `agent_judgement.md` 作为分析输入。`agent_judgement.md` 是输出产物，不能作为证据、导航、历史参考或 cause 依据；新 run 不再生成 `agent_brief.md`。
3. 建立 RAG stage map：把真实 workflow 节点和 trace spans 映射到 query/input、preprocess、recall、rerank、prompt/context、generation、postprocess/final_output、evaluator/judged_object。
4. 抽取 answer symptoms。
5. 根据表象列出候选根因。
6. 对每个候选根因写支持证据和反证证据。
7. 规划或执行 recall / rerank / replay / knowledge-detail 实验。
8. 运行 `synthesize-brief` 生成每条 case 的短版 `agent_judgement.md` 和 `evidence_index.json`。
9. 给用户写短版 judgement 回复，不要把 JSON 或长证据清单当作最终输出。

旧 `agent_brief.md` 和 `agent_judgement.md` 不是取证入口。已有 Markdown 不能用于检查、补证、继承 cause 或判断 stale；如果需要重新分析，直接基于当前 JSON 事实包重判。

## Workflow 节点与 RAG 阶段

不要把 workflow 节点当成最终分析对象。节点是 trace 定位索引，RAG 阶段产物才是归因对象。Agent 要用模型能力读节点名、类型、input/output、字段名、上下游边和 raw span 内容，判断某个节点到底承载了哪个 RAG 阶段。

RAG stage map 至少要回答：

- 当前问题的真实输入是什么，是否经过 rewrite、意图分类或条件分支。
- 召回产物来自哪些节点/API/分支，文档召回和 FAQ/精选召回是否要分开看。
- 重排前后同一 required assertion 的证据是否保留，rank/score 是否变化。
- prompt/context 的真实边界在哪里：知商问答节点、大模型节点、脚本拼 prompt 节点，或模型 input/messages。
- 生成答案和后处理包装分别发生在哪里，最终被评估的是哪个输出。

常见误区：

- 看到 `prompt_observation=not_observed` 就写“全部证据被过滤”。这只表示解析器没看到标准 prompt 字段；必须回查大模型、知商问答、脚本和后处理节点的 input/output。
- 看到脚本节点就跳过。脚本可能只是规范化，也可能负责 merge、拼 prompt、裁剪 evidence、包装答案或生成最终输出。
- 看到标准 RAG 节点名就停止深挖。自定义 workflow 可能把 prompt docs 放在模型节点，或把 answer 在后处理节点改写。
- 召回型 workflow 没有 LLM prompt 时还按问答链路归因。此时重点是候选覆盖、分支路由、merge/rerank 和最终候选输出。

如果已有 Markdown 结论与当前 JSON 证据或 stage map 冲突，忽略 Markdown；当前归因只以 JSON 事实包和实验 JSON 为准。
...
`agent_judgement.md` 不写面向 Agent 的报告要求或 checklist；这些规则留在 `SKILL.md` 和 `report_contract.md`。
