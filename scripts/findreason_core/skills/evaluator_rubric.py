...
def run_evaluator_rubric_skill(request: AttributionRequest) -> tuple[list[StageVerdict], list[DiagnosticResult], ReferenceChainStep]:
    results = run_diagnostic_spec("evaluator_rubric", request)
    verdicts = stage_verdicts_from_results(results)
    step = diagnostic_step_from_results(
        name="Evaluator / Rubric 诊断 Skill",
        request=request,
        results=results,
        skill_input={
